...
- The mavlink-battery-status-runtime-replay case uses a real PX4 runtime replay runner. Use --target pre or post. When CONTACT_DEPARTURE_REMOTE_RUNTIME is enabled, px4-runtime-replay jobs dispatch to the configured pod-backed Linux host while the project CLI and run folders stay the same locally. Summarize runtime_clean, runtime_anomalous, or runtime_unavailable, the resolved commit hash, frame delivery, sanitizers_used when relevant, artifact paths, and the runtime-replay-only caveat. Do not claim firmware safety or vulnerability discovery. To demonstrate a verdict flip, launch one pre and one post replay job with the same case and mode (use --mode asan when the host can build PX4 with AddressSanitizer), then pair the two completed job IDs.
