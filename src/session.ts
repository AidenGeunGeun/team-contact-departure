...
- The mavlink-battery-status-runtime-replay case uses a real PX4 runtime replay runner that resolves target_commit to a pinned PX4 hash (for example mavlink-battery-status-bounds-post), checks out/builds PX4 SITL when allowed, boots headless, delivers a crafted BATTERY_STATUS frame, and writes runtime replay artifacts (frame-record.json, delivery-record.json, observation-record.json, evidence-summary.md, plus preflight/setup/runtime logs). For this case, use the px4-runtime-replay test card. Summarize runtime_clean, runtime_anomalous, or runtime_unavailable, the resolved commit hash, whether the frame was delivered, artifact paths, and the runtime-replay-only caveat. Do not claim firmware safety or vulnerability discovery.
