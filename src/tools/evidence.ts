...
function formatEvidencePairComparison(details: CompareEvidencePairDetails): string {
  ...
  const lines = [
    ...
    `- roles correctly assigned: ${pair.roles_correctly_assigned ? "yes" : "no"}`,
    ...
    `- sanitizers_used equal: ${pair.sanitizers_used_equal ? "yes" : "no"}`,
    ...
    `- sanitizers_used: [${(pair.pre_patch.sanitizers_used ?? []).join(", ") || "none"}]`,
    "",
    "Post-patch job:",
    ...
    `- sanitizers_used: [${(pair.post_patch.sanitizers_used ?? []).join(", ") || "none"}]`,
    "",
